...
function renderArticleList() {
  ...
  articleState.filtered.forEach(function(a) {
    ...
    html += '<button class="article-item' + active + '" onclick="selectArticleFromList(' + a.id + ')"><div class="article-item-title">' + esc(a.title || 'Untitled') + '</div><div class="article-meta">' + esc(a.url_id || 'no-slug') + ' • ' + esc(a.author || 'No writer') + '</div><div class="article-meta">' + esc(a.status || 'draft') + (dateText ? ' • ' + esc(dateText) : '') + '</div></button>';
...
function renderArticleWorkspace() {
  ...
  if (subtitleEl) subtitleEl.textContent = (current.url_id ? ('/' + current.url_id) : 'New article draft') + ' • ' + (current.status || 'draft');
...
function autoSave() {
  ...
  savePromise.then(function() {
    ...
    setArticleSaveStatus('Auto-save failed — draft stored locally', true);
